...
def parse_option():
    ...
    # dataset
    parser.add_argument('--dataset', type=str, default='imagenet100', choices=[ 'miniimagenet','imagenet100', 'imagenet'])
    parser.add_argument('--data_folder', type=str, default='../miniimagenettools/processed_images/')
...
def train_moco(epoch, train_loader, model, model_ema, contrast, criterion, optimizer, opt):
    ...
    for idx, (inputs, _, index) in enumerate(train_loader):
        ...
        # out = contrast(feat_q, feat_k.transpose(1,0))###CHANGED
